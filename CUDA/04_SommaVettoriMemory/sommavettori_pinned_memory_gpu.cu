...
int main( void ) {
    ...
    HANDLE_ERROR(cudaHostAlloc( (void**)&dev_a, N * sizeof(int),0));        // 0 = default mode flag
    HANDLE_ERROR(cudaHostAlloc( (void**)&dev_b, N * sizeof(int),0));
    HANDLE_ERROR(cudaHostAlloc( (void**)&dev_c, N * sizeof(int),0));
    ...
    cudaDeviceSynchronize(); // per equità del test con il profiler rispetto alla zero copy che richiede il sync
